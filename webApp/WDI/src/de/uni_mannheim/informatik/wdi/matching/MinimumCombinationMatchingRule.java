 ...
 */
/**
 * A {@link MatchingRule} that evaluates to the lowest confidence in the group.
 *
 ...
 * @param <RecordType>
 */
public class MinimumCombinationMatchingRule<RecordType extends Matchable, SchemaElementType>
     ...
     */
    public MinimumCombinationMatchingRule(double finalThreshold) {
     ...
     * @param finalThreshold
     */
    public MinimumCombinationMatchingRule(double offset, double finalThreshold) {
        this(finalThreshold);
        this.offset = offset;
    }
    ...
    public Correspondence<RecordType, SchemaElementType> apply(RecordType record1, RecordType record2, ResultSet<Correspondence<SchemaElementType, RecordType>> schemaCorrespondences) {
        ...
        for (int i = 0; i < comparators.size(); i++) {
            ...
            if (isFirst) {
                lowest = similarity * weight;
                //wLowest = weight;
                isFirst = false;
            } else {
                if ((similarity * weight) < lowest) {
                    lowest = similarity * weight;
                    //wLowest = weight;
                }
            }
        ...
        //double similarity = offset + (lowest / wLowest);
        double similarity = offset + lowest;
